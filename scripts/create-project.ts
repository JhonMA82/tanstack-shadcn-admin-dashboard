...
/**
 * Proof that an existing directory was produced by this boilerplate: a
 * parseable `.boilerplate.json` marker with the expected schema and
 * template. Anything else (foreign checkouts, user data) must never be
 * deleted by `--force`.
 */
async function isBoilerplateGeneratedProject(destination: string): Promise<boolean> {
  let marker: BoilerplateMarker;
  try {
    marker = JSON.parse(await readFile(path.join(destination, ".boilerplate.json"), "utf8"));
  } catch {
    return false;
  }

  return (
    marker !== null && typeof marker === "object" && marker.schemaVersion === 1 && marker.template === "studio-admin"
  );
}

async function freshStagingDirectory(destination: string): Promise<string> {
  for (let attempt = 0; attempt < 10; attempt += 1) {
    const staging = `${destination}.tmp-${randomBytes(4).toString("hex")}`;
    if (!(await pathExists(staging))) {
      return staging;
    }
  }

  throw new Error(`Unable to allocate a staging directory next to ${destination}.`);
}
...
export async function createProject(options: CreateProjectOptions): Promise<string> {
  ...
  if (destinationExisted) {
    ...
    if (!(await isBoilerplateGeneratedProject(destination))) {
      throw new Error(
        `Refusing --force: destination exists and was not generated by this boilerplate: ${destination}. ` +
          "Delete it manually if replacement is really intended.",
      );
    }
  }

  // Materialize atomically: every generation step targets a sibling staging
  // directory, and the original destination is only replaced (when the
  // force-replace above was authorized) once all steps succeed. On failure
  // the staging directory is removed and the original is left untouched.
  const staging = await freshStagingDirectory(destination);

  try {
    await copyRepository(repositoryRoot, staging);

    const { sourceVersion } = await updatePackageIdentity(staging, packageName);
    ...
    await writeJson(path.join(staging, ".boilerplate.json"), {
      schemaVersion: 1,
    ...
    if (profile === "minimal") {
      await applyMinimalProfile(staging);
    }
    ...
    if (profile === "minimal") {
      await regenerateMinimalRouteTree(repositoryRoot, staging);
    }

    await applyDerivedProjectCleanup(staging);

    await generateAiContext({ repositoryRoot: staging });
